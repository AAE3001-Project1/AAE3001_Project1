...
%--------------------------------------------------------------------------
% Rankine Oval Fitting

% hold on;
p = polyfit(x_plot_oval(1:mid_point), y_plot_oval(1:mid_point), 50);
x_plot = linspace(-a, a, 1000);
y_plot = polyval(p, x_plot);
x_plot = [-a, x_plot, a, -x_plot, -a];
y_plot = [0, y_plot, 0, -y_plot, 0];
plot(x_plot,y_plot, 'Color', [128/255, 0, 128/255], 'LineWidth',1.5);

save 'fitting_coeff.txt' -ascii p;
dlmwrite('fitting_coeff_high_precision.txt', p, 'precision', '%.20f');

%--------------------------------------------------------------------------


title('Rankine Oval Fitting');
% xlabel('x');
% ylabel('y');
...
% legend('Original Rankine Oval', 'Fitting Rankine Oval');
