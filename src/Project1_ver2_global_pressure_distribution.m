...
x_plot = -4:precision_surf:4;
y_plot = -4:precision_surf:4;
pressure = 1 - (V_func(x_plot, y_plot') / V_inf).^2;

% for j = 1:length(y_plot)
%     pressure(j, :) = 1 - (V_func(x_plot, y_plot(j)) / V_inf);
%     j
% end

surf(x_plot, y_plot, pressure, pressure);
clim([-2 1])

hold on;
% h = contour(x_plot, y_plot, pressure, 150);
% clabel(h);
% [dx, dy]=gradient(pressure,.2,2);
% quiver(x_plot,y_plot,dx,dy);
% hold on;
%--------------------------------------------------------------------------



title('Global Pressure Distribution');
% xlabel('x');
% ylabel('y');
...
function V_f = V_func(x, y)
    V_f = sqrt((10+(10/pi)*((x+1)./((x+1).^2+y.^2) - (x-1)./((x-1).^2+y.^2))).^2 + (10/pi)*(y./((x+1).^2+y.^2) - y./((x-1).^2+y.^2)).^2);
end
