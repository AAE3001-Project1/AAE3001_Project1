...
x_plot_oval = [x_plot_oval, x_plot, -a];
y_plot_oval = [y_plot_oval, y_plot, 0];
...
l1 = y_plot(1):precision_yplot:y_plot(1)+precision_yplot*(line_num/2-1);
l2 = -y_plot(1):-precision_yplot:-y_plot(1)-precision_yplot*(line_num/2-1);
% for y_i = y_plot:precision_yplot:-y_plot
for y_i = [l1, l2]
    x_plot = [x_begin];
    y_plot = [y_i];
    for x_i = x_plot(1) + precision_xplot:precision_xplot:-x_plot(1)
        streamLine_diff = streamLine_diff_func(x_plot(end), y_plot(end));
        if isnan(streamLine_diff)
            streamLine_diff = v_func(x_plot(end-1), y_plot(end-1)) / u_func(x_plot(end-1),y_plot(end-1));
        end
        % fprintf('x: %.5f || y: %.5f || diff: %.5f || V: %.5f\n', x_plot(end), y_plot(end), streamLine_diff, V());
        y_plot = [y_plot, y_plot(end) + streamLine_diff * precision_xplot];
        x_plot = [x_plot, x_i];
    end
...
% pressure = C_p(x_plot_oval, y_plot_oval);
% plot3(x_plot_oval, y_plot_oval, pressure,'Color', [0, 1, 0]);
% hold on;
...
% colormap(hsv)
for i = 1:length(x_plot_oval)/draw_space
    
    i_d = i * draw_space;
    x_plot(end + 1) = x_plot_oval(i_d);
    y_plot(end + 1) = y_plot_oval(i_d);
    z_plot(end + 1) = C_p(x_plot_oval(i_d), y_plot_oval(i_d));
    % plot3([x_plot(i); x_plot(i)], [y_plot(i); y_plot(i)], [0; z_plot(i)],'Color', [0, (z_plot(i)+1.5)/3, (1.5-z_plot(i))/3]);

    line_num = floor((z_plot(end) + 1.5) / line_space);
    pressure = linspace(0, z_plot(i), line_num);
    patch(repelem(x_plot(i), line_num), repelem(y_plot(i), line_num), pressure, pressure, 'FaceColor','none','EdgeColor','interp')
...
plot3(x_plot, y_plot, z_plot, 'Color', [0, 1, 0], 'MarkerSize', 3);
...
title('Pressure Distribution & Stream Line');
% xlabel('x');
% ylabel('y');
